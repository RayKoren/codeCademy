...
function updateComment(url, request){
  ...
  if (!id || !requestComment || savedComment == requestComment) {
    response.status = 400;
  } else if (!savedComment) {
    response.status = 404;
  } else if (requestComment && savedComment){
...
if (!id || !delComment) {
  ...
  } else if (id && delComment) {

    database.comments[id] = null;

    let uID = database.users[delComment.username].commentIds
    uID.splice(uID.indexOf(id), 1);

    let aID = database.articles[delComment.articleId].commentIds
    aID.splice(aID.indexOf(id), 1);
    response.status = 204;
